...
class DataProcessor(object):
    ...
    def get_train_examples(self, image_captions):
        """Gets a collection of `InputExample`s for the train set."""
        raise NotImplementedError()

    def get_dev_examples(self, image_captions):
        """Gets a collection of `InputExample`s for the dev set."""
        raise NotImplementedError()
        
    def get_test_examples(self, image_captions):
...
class CaptionProcessor(DataProcessor):
    ...
    def get_train_examples(self, image_captions):
        """See base class."""
        return self._create_examples(image_captions,"TRAIN")

    def get_dev_examples(self, image_captions):
        """See base class."""
        return self._create_examples(image_captions,"VAL")
    
    def get_test_examples(self, image_captions):
        """See base class."""
        return self._create_examples(image_captions,"TEST")

    def get_labels(self):
        """See base class."""
        return [None]

    #training, validation, test captions로 나눠서 들어감
    def _create_examples(self,img_captions, set_type):
        examples = []
        for(img_i, captions) in enumerate(img_captions):
            for(cap_i, caption) in enumerate(captions):#5개
                guid = "%s-%s-%s" %(set_type, img_i, cap_i)
                text_a = caption # string
