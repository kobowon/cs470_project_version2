...
def train(args, train_dataset, model, tokenizer):
    ...
    args.eval_steps = len(train_dataloader)//20    
    args.logging_steps = args.eval_steps
    

    #bowonko <2019.10.7>
    #freeze할 parameter를 제외한 parameter list 
    remove_freeze_params_list= []
    for n,p in model.named_parameters():
        remove_freeze_params_list.append((n,p))
        #if('bert.embeddings' in n):
        #    continue
        #else:
        #    remove_freeze_params_list.append((n,p))
    print('##############################################################')
    print([n for n,p in remove_freeze_params_list])       
        
    # Prepare optimizer and schedule (linear warmup and decay)
    no_decay = ['bias', 'LayerNorm.weight']
    optimizer_grouped_parameters = [
        {'params': [p for n, p in remove_freeze_params_list if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in remove_freeze_params_list if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
        ]
    
    #parameter에서 freeze할 대상 
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training and global_step % args.eval_steps == 0:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        #bunbun
                        #여기서 checkpoint 저장
                        results = evaluate(args, model, tokenizer,global_step,curr_epoch+1) #2019.09.27 global step 추가
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    # Save model checkpoint every save_step
...
def evaluate(args, model, tokenizer, prefix="",curr_epoch=0):
    ...
    if (curr_f1 > best_f1):
        ...
        #train > evaluation 위치로 변경
        
        global_step = prefix
        output_checkpoint_dir = os.path.join(args.output_dir, 'checkpoint-{}-{}'.format(args.best_epoch,global_step))
        ...
        if (args.best_f1 > 1):
            ...
            with open(output_eval_file,"w") as writer:
                eval_result = {}
                eval_result['f1'] = args.best_f1
                eval_result['epoch'] = args.best_epoch
                for key in sorted(eval_result.keys()):
                    writer.write("%s = %s\n" %(key, str(eval_result[key])))
    
    else :
        args.patient -= 1
    print('현재 최고 기록\n','best f1 : ',args.best_f1, 'best_epoch : ',args.best_epoch)
    return results
...
def main():
    ...
    parser.add_argument('--logging_steps', type=int, default=3000,#수정
    ...
    parser.add_argument('--best_f1', type=float, default=0.,
                        help="best f1 score")    
    parser.add_argument('--best_epoch', type=float, default=0.,
                        help="best epoch")  
    parser.add_argument('--patient', type=float, default=50.,
                        help="early stopping")  
